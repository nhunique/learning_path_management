...
<body>
    ...
    <h1 class="container-heading">Update Your Clone Learning Path</h1>
    <?php
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['update'])) {
            ...
            $clonepaths = $learningPath->getSpecificClonePath($cloneID);
            var_dump($clonepaths);

            $cloneurls = $learningPath->getUrlsByCloneID($cloneID);
            $numOfUrls = count($cloneurls);

            foreach ($clonepaths as $path) {
            ...
            // Form starts here
            ?>
            <form id="updateForm" method="post" action="update-clone-handler.php">
                <fieldset>
                    ...
                    <div class="form-group" id="urlFields">
                        ...
                        foreach ($cloneurls as $num => $url) {
                            $urlTitle = $cloneurls[$num]['urlTitle'];
                            $urlLink = $cloneurls[$num]['urlLink'];
